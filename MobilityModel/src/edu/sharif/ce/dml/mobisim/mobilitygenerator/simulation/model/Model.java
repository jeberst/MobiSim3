...
public abstract class Model extends ParameterableImplement {
   ...
   public int getCoverage()
    ...
    public List<GeneratorNode> CreateSensorCluster (List<GeneratorNode> nodes)
    {
       if(nodes.size() % 5 == 0)
       {
        //clusteres are created clockwise staring in the middle
        GeneratorNode n = nodes.get(0);
       SensorNode center = new SensorNode(n, true);
       
       SensorNode top = new SensorNode(nodes.get(1), false, null, null, center, null);
       Cluster.add(top);
       SensorNode right = new SensorNode(nodes.get(2),true, null, null, null, center);
       Cluster.add(right);
       SensorNode bottom = new SensorNode(nodes.get(3), false, center, null, null, null);
       Cluster.add(bottom);
       SensorNode left = new SensorNode(nodes.get(4), false, null, center, null, null);
       Cluster.add(left);
